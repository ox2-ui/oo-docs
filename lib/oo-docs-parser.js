	...
	var dataObject = ooDocs[params.packageOrg][params.packageName].versions[params.packageVersion].api;
		for (i in dataObject) {
			PackageApiTypes.insert({
				...
				apiType: dataObject[i].title,
				apiDocs: dataObject[i].docs,
			}, function( err, res) {
				if (err)
					console.log(err)
				else {
					for (j in dataObject[i].params) {
						console.log(dataObject[i].params[j])
						PackageApi.insert({
							packageId: params.packageId,
							apiTypeId: res,
							apiName: dataObject[i].params[j].apiName,
							apiValue: dataObject[i].params[j].apiValue,
							description: dataObject[i].params[j].description,
						})
					}
				}
			})
